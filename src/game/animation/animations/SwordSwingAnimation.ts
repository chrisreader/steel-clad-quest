...
export class SwordSwingAnimation {
  ...
  public update(): void {
    ...
    if (elapsed < phases.windup) {
      // WINDUP PHASE: Move arm dramatically to the right and coil
      ...
      // DRAMATIC SPATIAL MOVEMENT - move arm far to the right with arc
      shoulderPosition.x = THREE.MathUtils.lerp(0, 1.2, easedT);     // Move dramatically to the right
      shoulderPosition.y = THREE.MathUtils.lerp(0, 0.4, easedT);     // Raise arm up for overhead position
      shoulderPosition.z = THREE.MathUtils.lerp(0, -0.4, easedT);    // Pull back for maximum coiling
      
      // Elbow - retract arm dramatically during windup
      elbowRotation.x = THREE.MathUtils.lerp(-0.05, -1.2, easedT);   // Bend arm in much more
      elbowRotation.y = THREE.MathUtils.lerp(0, 0.2, easedT);
      ...
      torsoRotation = THREE.MathUtils.lerp(0, -0.7, easedT);
      
    } else if (elapsed < phases.windup + phases.slash) {
      // SLASH PHASE: Sweep arm dramatically from right to far left across body
      ...
      // DRAMATIC CROSS-BODY SWEEP - from far right to far left with arc
      shoulderPosition.x = THREE.MathUtils.lerp(1.2, -1.2, easedT);    // Massive sweep across body
      shoulderPosition.y = THREE.MathUtils.lerp(0.4, -0.2, easedT);    // Arc downward as swing progresses
      shoulderPosition.z = THREE.MathUtils.lerp(-0.4, 0.8, easedT);    // Extend forward dramatically into strike
      
      // Elbow - explosive extension for maximum reach and power
      elbowRotation.x = THREE.MathUtils.lerp(-1.2, 0.5, easedT);      // Straighten arm completely for reach
      elbowRotation.y = THREE.MathUtils.lerp(0.2, -0.3, easedT);
      ...
      // Enhanced torso uncoiling with more power
      torsoRotation = THREE.MathUtils.lerp(-0.7, 1.0, easedT);
      ...
      shoulderPosition.x = THREE.MathUtils.lerp(-1.2, 0, easedT);      // Return from far left to center
      shoulderPosition.y = THREE.MathUtils.lerp(-0.2, 0, easedT);      // Return to neutral height
      shoulderPosition.z = THREE.MathUtils.lerp(0.8, 0, easedT);       // Return from extended to neutral
      
      // Return elbow to ready position
      elbowRotation.x = THREE.MathUtils.lerp(0.5, -0.05, easedT);
      ...
      torsoRotation = THREE.MathUtils.lerp(1.0, 0, easedT);
  ...
  private applyRotations(
    ...
    // Apply dramatic spatial shoulder movement
