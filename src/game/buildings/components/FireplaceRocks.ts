...
export class FireplaceRocks {
  ...
  public createRockCircle(radius: number = 0.8, rockCount: number = 8): THREE.Group {
    ...
    for (let i = 0; i < rockCount; i++) {
      ...
      // Generate realistic campfire-sized rocks - prefer boulders for natural look
      const rockType = Math.random() < 0.6 ? 'boulder' : Math.random() < 0.8 ? 'flat' : 'angular';
      const rockShape = SimpleRockGenerator.generateSimpleRock(rockType, 0.12 + Math.random() * 0.06);

      // Create rock with realistic campfire scale
      const rock = new THREE.Mesh(rockShape.geometry, rockMaterial.clone());
      const scaleValue = 0.8 + Math.random() * 0.4; // 0.8 to 1.2 scale
      ...
      rock.position.set(x, 0.08 + heightVariation, z);
    ...
    // Add smaller accent rocks for natural variation
    for (let i = 0; i < 3; i++) {
      ...
      const smallRockShape = SimpleRockGenerator.generateSimpleRock('flat', 0.08);
      const smallRock = new THREE.Mesh(smallRockShape.geometry, rockMaterial.clone());
      
      smallRock.scale.setScalar(0.5 + Math.random() * 0.3);
      smallRock.rotation.copy(smallRockShape.rotation);
      smallRock.position.set(x, 0.06, z);
    ...
    console.log(`🪨 Realistic fireplace rock circle created with ${this.rocks.length} properly sized rocks`);
