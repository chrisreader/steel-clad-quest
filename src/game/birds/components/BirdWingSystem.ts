...
import { WingSegments, BirdMaterialSet } from '../core/BirdTypes';
import { BirdMaterials } from '../core/BirdMaterials';

export class BirdWingSystem {
  ...
  public createWings(): { 
    ...
    const leftWingData = this.createAnatomicalWing(true);
    const rightWingData = this.createAnatomicalWing(false);
    
    leftWingGroup.add(leftWingData.wingGroup);
    rightWingGroup.add(rightWingData.wingGroup);

    return {
      leftWingGroup,
      rightWingGroup,
      wingSegments: {
        left: leftWingData.segments,
        right: rightWingData.segments
      }
    };
  }

  private createAnatomicalWing(isLeft: boolean): { wingGroup: THREE.Group; segments: WingSegments } {
    ...
    // Create feathers
    const covertFeathers = this.createCovertFeathers(humerusGroup, side);
    const secondaryFeathers = this.createSecondaryFeathers(forearmGroup, side);
    const primaryFeathers = this.createPrimaryFeathers(handGroup, side);
    ...
    return { wingGroup, segments };
  }

  private createCovertFeathers(humerusGroup: THREE.Group, side: number): THREE.Mesh[] {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      const featherGeometry = BirdMaterials.createTaperedFeatherGeometry(baseWidth, tipWidth, featherLength);
  ...
  private createSecondaryFeathers(forearmGroup: THREE.Group, side: number): THREE.Mesh[] {
  ...
  private createPrimaryFeathers(handGroup: THREE.Group, side: number): THREE.Mesh[] {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      // Orient feathers to point backward along bird's body (negative X)
      feather.rotation.x = 0; // No rotation around X since feathers are X-aligned
      feather.rotation.y = -(0.2 - i * 0.008); // Flip Y rotation for flipped geometry
      feather.rotation.z = (i * 0.03); // Gradual lift toward wingtip
