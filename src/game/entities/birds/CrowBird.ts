...
export class CrowBird extends BaseBird {
  ...
  private createAnatomicalWing(isLeft: boolean): THREE.Group {
    ...
    // Simple wing bone structure
    const wingBone = new THREE.Group();
    const boneGeometry = new THREE.CapsuleGeometry(0.02, 0.4, 6, 8);
    const bone = new THREE.Mesh(boneGeometry, this.materials!.leg);
    bone.rotation.z = Math.PI / 2; // Orient along Z-axis
    bone.position.set(0, 0, side * 0.2);
    wingBone.add(bone);
    wingGroup.add(wingBone);

    // Inner wing surface (shoulder to mid-wing)
    const innerWingGeometry = new THREE.PlaneGeometry(0.3, 0.25);
    const innerWing = new THREE.Mesh(innerWingGeometry, this.materials!.feather);
    innerWing.position.set(-0.1, 0, side * 0.15);
    innerWing.rotation.y = side * Math.PI / 2; // Face outward
    wingGroup.add(innerWing);

    // Outer wing surface (mid-wing to tip) - narrower
    const outerWingGeometry = new THREE.PlaneGeometry(0.25, 0.15);
    const outerWing = new THREE.Mesh(outerWingGeometry, this.materials!.feather);
    outerWing.position.set(-0.08, 0, side * 0.35);
    outerWing.rotation.y = side * Math.PI / 2; // Face outward
    wingGroup.add(outerWing);

    // Wing tip - pointed
    const tipGeometry = new THREE.ConeGeometry(0.08, 0.12, 6);
    const wingTip = new THREE.Mesh(tipGeometry, this.materials!.feather);
    wingTip.rotation.x = Math.PI / 2;
    wingTip.rotation.z = side * Math.PI / 2;
    wingTip.position.set(-0.06, 0, side * 0.45);
    wingGroup.add(wingTip);

    // Store simplified wing references
    const segments: WingSegments = {
      upperArm: bone,
      forearm: bone, // Simplified - same bone
      hand: bone, // Simplified - same bone
      primaryFeathers: [outerWing, wingTip],
      secondaryFeathers: [innerWing],
      covertFeathers: []
    };
    ...
    // Store wing parts for animation
    wingGroup.userData = {
      innerWing,
      outerWing,
      wingTip,
  ...
  private animateWings(): void {
    ...
    // Apply simple wing animations based on state
    switch (this.birdState) {
      case BirdState.IDLE:
      case BirdState.WALKING:
      case BirdState.FORAGING:
        // Wings folded against body
        leftWingGroup.rotation.set(0, 0, -0.8);
        rightWingGroup.rotation.set(0, 0, 0.8);
        break;
        
      case BirdState.ALERT:
        // Slight wing lift
        leftWingGroup.rotation.set(0, 0, -0.4);
        rightWingGroup.rotation.set(0, 0, 0.4);
        break;
        
      case BirdState.TAKING_OFF:
        // Powerful flapping
        const takeoffBeat = Math.sin(this.flapCycle) * 0.8;
        leftWingGroup.rotation.set(takeoffBeat, 0, -0.2 + takeoffBeat * 0.3);
        rightWingGroup.rotation.set(-takeoffBeat, 0, 0.2 - takeoffBeat * 0.3);
        break;
        
      case BirdState.FLYING:
        // Regular flapping
        const flapBeat = Math.sin(this.flapCycle) * 0.5;
        leftWingGroup.rotation.set(flapBeat, 0, -0.1 + flapBeat * 0.2);
        rightWingGroup.rotation.set(-flapBeat, 0, 0.1 - flapBeat * 0.2);
        break;
        
      case BirdState.SOARING:
        // Wings fully extended
        leftWingGroup.rotation.set(0.1, 0, 0);
        rightWingGroup.rotation.set(-0.1, 0, 0);
        break;
        
      case BirdState.LANDING:
        // Wings spread for braking
        leftWingGroup.rotation.set(0.3, 0, 0.2);
        rightWingGroup.rotation.set(-0.3, 0, -0.2);
        break;
    }
  }
