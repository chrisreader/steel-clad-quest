...
export class Player {
  ...
  private initializeBowAnimationPositions(): void {
    ...
    this.bowDrawAnimation.leftHandRestPosition.set(-0.5, 1.6, -0.5); // Changed from -0.9 to -0.5 units to the left
    ...
    this.bowDrawAnimation.leftHandDrawPosition.set(-0.5, 1.6, -0.5); // Changed from -0.9 to -0.5 - Left hand stays steady on bow
    ...
    console.log("🏹 [Player] FIXED: Left arm positioned 0.5 units to the left and pushed back 50% more (2.925x) to hide cylinder top from first-person view");
  }
  ...
  private setArcheryStance(): void {
    // Set both arms to archery ready position
    this.playerBody.leftArm.position.copy(this.bowDrawAnimation.leftHandRestPosition);
    this.playerBody.rightArm.position.copy(this.bowDrawAnimation.rightHandRestPosition);
