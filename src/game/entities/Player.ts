...
export class Player {
  ...
  public equipWeapon(weaponId: string): boolean {
    ...
    if (this.isBowEquipped) {
      ...
      // UPDATED: Position bow handle properly with X: 30°, Y: 0°, Z: -85° rotation
      weapon.getMesh().position.set(0, -0.05, 0); // Position relative to left hand
      weapon.getMesh().rotation.set(Math.PI * 30 / 180, 0, -Math.PI * 85 / 180); // UPDATED: 30° X, 0° Y, -85° Z
      weapon.getMesh().scale.set(1.0, 1.0, 1.0);
      
      // Set bow ready stance - left arm raised outward
      this.setWeaponArmStance('bow');
      
      console.log(`🏹 [Player] Bow equipped with UPDATED 30° X, 0° Y, -85° Z rotation and raised left arm ready stance`);
    } else {
      weaponType = 'melee';
