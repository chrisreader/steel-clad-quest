...
export class Player {
  ...
  public equipWeapon(weaponId: string): boolean {
    ...
    if (this.isBowEquipped) {
      ...
      // FIXED: Clean bow rotation - X: 30° forward tip, Y: 0° no twist, Z: 0° no tilt
      weapon.getMesh().position.set(0, -0.05, 0); // Position relative to left hand
      weapon.getMesh().rotation.set(Math.PI * 30 / 180, 0, 0); // FIXED: 30° X, 0° Y, 0° Z
      weapon.getMesh().scale.set(1.0, 1.0, 1.0);
      
      // Set bow ready stance - left arm raised outward
      this.setWeaponArmStance('bow');
      
      console.log(`🏹 [Player] Bow equipped with FIXED clean rotation 30° X, 0° Y, 0° Z - no twist or tilt`);
    } else {
      weaponType = 'melee';
